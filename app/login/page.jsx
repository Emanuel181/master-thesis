'use client'

import { motion } from 'motion/react';
import { LoginForm } from "@/components/login-form" // Ensure this path is correct for your project

export default function LoginPage() {
    return (
        <div className="grid min-h-svh lg:grid-cols-2">
            {/* Left Side: Login Form */}
            <div className="flex flex-col gap-4 p-6 md:p-10">
                <div className="flex justify-center gap-2 md:justify-start">
                    <a href="#" className="flex items-center gap-2 font-medium text-2xl">
                        <img src="https://amz-s3-pdfs-gp.s3.us-east-1.amazonaws.com/logo/logo.png" alt="Logo" className="w-10 h-10" />
                        VulnIQ
                    </a>
                </div>
                <div className="flex flex-1 items-center justify-center">
                    <div className="w-full max-w-xs">
                        <LoginForm />
                    </div>
                </div>
            </div>

            {/* Right Side: Agentic RAG Animation */}
            {/* We override bg-muted with a dark theme for the code visualization */}
            <motion.div
                className="relative hidden lg:flex flex-col items-center justify-center overflow-hidden border-l border-zinc-800 bg-[length:400%_400%]"
                style={{ backgroundColor: '#99ffa5', backgroundImage: 'linear-gradient(45deg, hsla(263,79%,77%,1), hsla(75,98%,73%,1), hsla(152,94%,68%,1), hsla(215,70%,75%,1), hsla(165,89%,79%,1), hsla(94,83%,71%,1), hsla(128,76%,79%,1))' }}
                animate={{ backgroundPosition: ['0% 50%', '100% 50%', '0% 50%'] }}
                transition={{ duration: 15, ease: 'easeInOut', repeat: Infinity }}
            >
            </motion.div>
        </div>
    )
}
