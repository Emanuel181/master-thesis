...
import { Github, RefreshCw, Search, ChevronLeft, ChevronRight, Info } from "lucide-react"
import { RepoCard } from "./repo-card"

/**
 * Hook that periodically tests the GitHub connection health.
 */
function useConnectionHealth(isConnected, isDemoMode) {
    const [health, setHealth] = useState({ status: "unknown", lastChecked: 0 }) // unknown | healthy | degraded | error
    const intervalRef = useRef(null)

    const checkHealth = useCallback(async () => {
        if (!isConnected || isDemoMode) return
        try {
            const res = await fetch("/api/github/repos?per_page=1", { method: "GET", cache: "no-store" })
            setHealth({
...
function ConnectionHealthIndicator({ health }) {
    const statusMap = {
        ...
        error: { dot: "bg-destructive", label: "Connection failed", detail: "Could not reach GitHub API" },
...
/**
 * GitHub repository panel component.
        ...
        <Card className={`flex flex-col overflow-hidden card-hover-lift min-h-[420px] ${className || ''}`}>
            ...
            <CardContent className="flex-1 min-h-0 overflow-hidden pt-2 pb-6 px-2.5 sm:px-4">
                {status === "loading" && !isDemoMode ? (
                    ...
                    <ConnectPrompt onConnect={connect} provider="GitHub" icon={Github} />
                ) : (
                    <RepoList
                        repos={repos}
                        isLoading={isLoading}
 ...
 */
function ConnectPrompt({ onConnect, provider, icon: Icon }) {
    return (
        <div className="h-full flex flex-col items-center justify-center text-center p-3 sm:p-4">
            <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-full bg-muted/30 flex items-center justify-center mb-3 sm:mb-4 border border-border/50">
                <Icon className="h-5 w-5 sm:h-7 sm:w-7 text-muted-foreground/60" />
            </div>
            <h3 className="text-sm font-medium mb-1">Connect {provider}</h3>
...
function RepoList({
    ...
    return (
        <div className="h-full flex flex-col min-h-0">
            ...
            {repos.length === 0 ? (
                <EmptyState icon={Icon} message={`No repositories found in your ${provider} account`} />
            ) : totalCount === 0 ? (
                <EmptyState 
                    icon={Search} 
                    title="No Results"
 ...
 */
function EmptyState({ icon: Icon, title = "No Repositories", message }) {
    return (
