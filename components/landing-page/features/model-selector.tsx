...
import { cn } from "@/lib/utils"; // Assuming you have this helper, or I can inline it
...
export function ModelSelector() {
    return (
        <Card>
            ...
            <CardTitle>Orchestrate Intelligence</CardTitle>
            <CardDescription>
                Select best-in-class models for reasoning or run local LLMs for total privacy.
            </CardDescription>
        </Card>
    );
}
        ...
        <div className="p-8 overflow-hidden h-full relative flex items-center justify-center">
            <div className="flex flex-row shrink-0 justify-center items-center gap-2">
                ...
                <Container className="h-12 w-12 circle-2">
                    <GoCopilot className="h-6 w-6 text-zinc-900 dark:text-white" />
                </Container>
                <Container className="circle-3">
                    <OpenAILogo className="h-8 w-8 text-zinc-900 dark:text-white" />
                </Container>
...
const Sparkles = () => {
    const [sparklesData, setSparklesData] = React.useState([]);

    React.useEffect(() => {
        setSparklesData([...Array(12)].map(() => ({
        ...
        <div className="absolute inset-0">
            {sparklesData.map((sparkle, i) => (
                <motion.span
                    key={`star-${i}`}
                    animate={{
                        top: `calc(${sparkle.initialTop}% + ${sparkle.moveTop}px)`,
                        left: `calc(${sparkle.initialLeft}% + ${sparkle.moveLeft}px)`,
...
export const Card = ({ className, children }: { className?: string; children: React.ReactNode }) => {
    return (
        <div
            className={cn(
                "max-w-sm w-full mx-auto p-8 rounded-xl border border-zinc-800 bg-zinc-900/50 shadow-2xl backdrop-blur-sm",
                className
            )}
        >
            {children}
...
export const CardDescription = ({ children, className }: { children: React.ReactNode; className?: string }) => {
    return (
        <p className={cn("text-sm font-normal text-zinc-400 max-w-sm", className)}>
            {children}
        </p>
    );
};
    ...
    return (
        <div
            className={cn(
                ...
                className,
                showGradient && "bg-zinc-800/40 [mask-image:radial-gradient(50%_50%_at_50%_50%,white_0%,transparent_100%)]"
            )}
        >
            {children}
        </div>
...
const Container = ({ className, children }: { className?: string; children: React.ReactNode }) => {
    return (
        <div
            className={cn(
                `h-16 w-16 rounded-full flex items-center justify-center bg-zinc-900/80
