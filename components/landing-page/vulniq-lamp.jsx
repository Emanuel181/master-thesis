    ...
    return (
        <LampContainer>
            <motion.h2
                ...
                className="mt-4 xs:mt-6 sm:mt-12 bg-gradient-to-br from-[var(--brand-light)] to-accent py-4 xs:py-6 sm:py-8 bg-clip-text text-center text-5xl xs:text-6xl sm:text-8xl md:text-9xl lg:text-[12rem] font-bold tracking-tight text-transparent px-4"
            >
                VulnIQ
            </motion.h2>
            ...
            <motion.p
                ...
                className="mt-4 xs:mt-6 sm:mt-8 text-foreground/70 text-center text-lg xs:text-xl sm:text-2xl md:text-3xl lg:text-4xl max-w-xs xs:max-w-sm sm:max-w-xl md:max-w-3xl lg:max-w-4xl px-4"
            >
            ...
            >
                <Button asChild size="lg" className="rounded-lg sm:rounded-xl font-semibold w-full sm:w-auto text-sm sm:text-base md:text-lg lg:text-2xl px-5 sm:px-6 md:px-8 lg:px-10 py-2.5 sm:py-3 md:py-5 lg:py-7 h-auto">
                    <a href={isAuthenticated ? "/dashboard" : "/login"} className="flex items-center justify-center gap-2 sm:gap-2.5 md:gap-3">
        ...
        <div
            ...
            <div className="relative flex w-full flex-1 scale-y-125 items-center justify-center isolate z-0">
                ...
                <motion.div
                    ...
                    transition={{ delay: 0.3, duration: 0.8, ease: "easeInOut" }}
                    className="absolute inset-auto z-30 h-36 w-64 -translate-y-[6rem] rounded-full bg-accent opacity-70 blur-2xl"
                />
                {/* Lamp line */}
                <motion.div
                    initial={{ width: "15rem" }}
