...
// Lazy-loaded default values (to avoid errors during build time)
// These are computed on first access at runtime, not at module load time
let _defaultRpID = null;
let _defaultOrigin = null;

function getDefaultRpID() {
    if (_defaultRpID === null) {
        _defaultRpID = getRpID();
    }
    return _defaultRpID;
}
...
// Log RP configuration on first use in development (lazy logging)
let _loggedConfig = false;
function logConfigOnce() {
    if (isDevelopment && !_loggedConfig) {
        _loggedConfig = true;
        log('[WebAuthn] Default RP ID:', getDefaultRpID());
        log('[WebAuthn] Default Origin:', getDefaultOrigin());
