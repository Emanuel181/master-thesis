...
export function verifyCodeOnly(email, providedCode) {
    ...
    if (!safeCompare(providedCode, storedData.code)) {
        storedData.attempts++;
...
export function verifyCodeAndGrantSession(email, providedCode) {
    ...
    // Verify code (or check if already verified, timing-safe comparison)
    if (!storedData.codeVerified && !safeCompare(providedCode, storedData.code)) {
        storedData.attempts++;
        return { valid: false, error: `Invalid code. ${5 - storedData.attempts} attempts remaining.` };
    }
