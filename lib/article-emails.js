 ...
 * Strips CRLF to prevent email header injection, then HTML-escapes
 */
...
async function sendEmail({ to, subject, html, text }) {
  try {
    ...
    // SECURITY: Only log email addresses in development to prevent PII exposure in production logs
...
export async function sendArticleRejectedEmail({ to, articleTitle, feedback }) {
  ...
  const content = `
    <p>Your article <strong>"${safeTitle}"</strong> has been reviewed and requires revision.</p>
    ${feedback ? `
    <div style="margin: 24px 0; padding: 16px; background-color: #fef2f2; border-left: 4px solid #ef4444; border-radius: 4px;">
      ...
      <p style="margin: 8px 0 0; color: #7f1d1d;">${safeFeedback}</p>
    </div>
    ` : ""}
  ...
  return sendEmail({
    to,
    subject: `Article Needs Revision: ${sanitizeForSubject(articleTitle)}`,
...
export async function sendArticleScheduledForDeletionEmail({ to, articleTitle, feedback, deletionDate }) {
  ...
  const content = `
    <p>Your article <strong>"${safeTitle}"</strong> has been scheduled for deletion.</p>
  ...
  return sendEmail({
    to,
    subject: `Article Scheduled for Deletion: ${sanitizeForSubject(articleTitle)}`,
    html: generateEmailTemplate({
...
export async function sendArticleDeletedEmail({ to, articleTitle, feedback }) {
  ...
  const content = `
    <p>Your article <strong>"${safeTitle}"</strong> has been permanently deleted.</p>
    ${feedback ? `
    <div style="margin: 24px 0; padding: 16px; background-color: #f1f5f9; border-left: 4px solid #64748b; border-radius: 4px;">
      ...
      <p style="margin: 8px 0 0; color: #64748b;">${safeFeedback}</p>
    </div>
    ` : ""}
  ...
  return sendEmail({
    to,
    subject: `Article Deleted: ${sanitizeForSubject(articleTitle)}`,
    html: generateEmailTemplate({
...
export async function sendArticleStatusChangedEmail({ to, articleTitle, newStatus, feedback }) {
  ...
  const statusInfo = statusMessages[newStatus] || {
    title: "Article Status Updated",
    message: `Your article status has been changed to ${safeStatus}.`,
  };

  const content = `
    <p>Your article <strong>"${safeTitle}"</strong> status has been updated.</p>
  ...
  return sendEmail({
    to,
    subject: `${statusInfo.title}: ${sanitizeForSubject(articleTitle)}`,
    html: generateEmailTemplate({
