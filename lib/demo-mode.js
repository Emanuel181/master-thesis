...
export function isDemoRequest(request) {
    // SECURITY: Check the middleware-sanitized header FIRST (most trusted source)
    const demoHeader = request.headers.get('x-vulniq-demo-mode');
    if (demoHeader === 'true') {
        return true;
    }

    // Fallback: Check referer header for requests that may not go through middleware
    // (e.g., browser navigation). Only used as secondary signal.
