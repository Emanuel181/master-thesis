...
export async function fetchRepoTree(owner, repo, provider = 'github') {
    ...
    const response = await fetch(`${endpoint}?owner=${owner}&repo=${repo}&shape=hierarchy`);
...
export async function fetchFileContent(owner, repo, path, provider = 'github') {
    ...
    const response = await fetch(`${endpoint}?owner=${owner}&repo=${repo}&path=${encodeURIComponent(path)}&decode=1`);
