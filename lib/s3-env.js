...
/**
 * Generate S3 key for article markdown content
 * @param {Env} env
 * @param {string} userId
 * @param {string} articleId
 */
export function generateArticleContentS3Key(env, userId, articleId) {
    const { prefix } = getS3Config(env);
    if (env === "demo") {
        return `${prefix}users/${userId}/articles/${articleId}/content.md`;
    }
    return `${prefix}${userId}/articles/${articleId}/content.md`;
}
 ...
 */
export async function uploadMarkdownToS3(env, key, markdown) {
    assertKeyPrefix(env, key);
    const { client, bucket } = getS3Config(env);
 ...
 */
export async function downloadMarkdownFromS3(env, key) {
    assertKeyPrefix(env, key);
    const { client, bucket } = getS3Config(env);
    const command = new GetObjectCommand({ Bucket: bucket, Key: key });
    const response = await client.send(command);
...
/**
 * Get an S3 object as a readable stream (for streaming into zip archives, etc.)
 * @param {Env} env
 * @param {string} key
 * @returns {Promise<import("stream").Readable>}
 */
export async function getS3ObjectStream(env, key) {
    assertKeyPrefix(env, key);
    const { client, bucket } = getS3Config(env);
    const command = new GetObjectCommand({ Bucket: bucket, Key: key });
    const response = await client.send(command);
