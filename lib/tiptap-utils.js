...
export function sanitizeUrl(inputUrl, baseUrl, protocols) {
  try {
    // First, check for dangerous protocols before even parsing
    // This catches obfuscated attempts like "javascript\t:" or "java\0script:"
    const normalizedInput = inputUrl.replace(ATTR_WHITESPACE, '').toLowerCase()
    for (const dangerous of DANGEROUS_PROTOCOLS) {
      if (normalizedInput.startsWith(dangerous + ':')) {
        return "#"
      }
    }
    ...
    // Double-check the parsed URL's protocol
