...
if (isProduction && !TOKEN_SECRET) {
    throw new Error('NEXTAUTH_SECRET must be set in production for unlock token signing');
}
...
export function createUnlockToken(userId) {
    ...
    const sig = crypto.createHmac('sha256', TOKEN_SECRET || 'dev-only-secret').update(payloadB64).digest('base64url');
...
export function verifyUnlockToken(token) {
    ...
    const expected = crypto.createHmac('sha256', TOKEN_SECRET || 'dev-only-secret').update(payloadB64).digest('base64url');
